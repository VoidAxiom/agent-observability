...
usage() {
  printf '%s\n' \
    'Usage: cc-launch.sh -- <command> [args...]' \
    '       cc-launch.sh --print-traceparent' >&2
}
...
resolve_traceparent() {
  if [[ "${TRACEPARENT:-}" =~ $TRACEPARENT_RE ]]; then
    ...
    TRACEPARENT_SOURCE='generated'
  fi
}

if [[ "$#" -gt 0 && "$1" == '--print-traceparent' ]]; then
  resolve_traceparent
...
if [[ "$#" -gt 0 && "$1" == '--' ]]; then
  ...
  printf 'cc-launch: TRACEPARENT=%s (%s)\n' "$TRACEPARENT" "$TRACEPARENT_SOURCE" >&2
  exec "$@"
fi
