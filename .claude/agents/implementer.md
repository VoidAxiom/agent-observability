...
- your worktree path (sibling of the primary checkout at `<repo-parent>/.agent-observability-worktrees/<name>/`)
...
<the EXACT command(s) from the packet spec — per-component: pytest sdk/ for
the Python SDK; otelcol-contrib validate --config collector/config.yaml for
the OTel Collector; clickhouse-client --multiquery --queries-file
clickhouse/schema.sql for ClickHouse schema; xcodebuild test -scheme
AgentObservability for the SwiftUI app; bash bin/<script> --dry-run for
launch wrappers>
...
Then run the project gates on the codex-produced working tree. The exact
commands depend on which component the packet touches; the packet spec
names them. Per-component defaults:

```bash
# Python SDK (sdk/**):
pytest sdk/ -q

# OTel Collector config (collector/**):
...
# plus the wrapper's own --dry-run if defined

# If the packet includes a runtime-verification step (per CLAUDE.md
# "Deliver a working product"), the impl runs that too and captures the
# observable output Claude will check at pre-PR.
```
...
- final gate output (the packet spec's verification command(s) — per-component pytest / otelcol-contrib validate / clickhouse-client / xcodebuild test / shell syntax check — plus the runtime-verification step output if the packet defined one)
