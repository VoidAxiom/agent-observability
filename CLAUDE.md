# agent-observability — agent operating guide

**agent-observability** — agent observability using otel and clickhouse
...
**agent_observability**) and in `docs/`.
...
1. **Linear** — issue under **agent_observability** (create if missing). Set
...
Per change:
- The packet spec's verification command(s) clean. Per-component defaults:
...
Worktrees sit at `<repo-parent>/.agent-observability-worktrees/<name>/`
...
**Source of truth: `docs/observability-spec.md`** (the design brief). The
spec carries open `DECISION NEEDED` markers; the locked decisions for this
build are recorded here. When the spec is updated, sync this section.

**Locked architectural decisions** (locked 2026-05-28):

- **Transport: A — OTel Collector (Go binary).** Stock collector receives
  OTLP, batches/redacts/retries, writes to ClickHouse via the official
  exporter. No Rust written. The spec's `<transport>/` directory resolves
  to `collector/`.
- **Emitter SDK: Python with auto-instrumentation.** Lean on
  `OpenAIInstrumentor().instrument()` (and equivalents) so spans appear
  for free. Accepted tradeoff: the magic can break silently on SDK
  upgrades — guard with smoke tests that prove a real span lands.
- **Daemon enrichment scope: N/A.** Transport=A means no daemon; the
  spec's third `DECISION NEEDED` is moot.

**Repo layout (target, per the spec — `collector/` chosen for `<transport>/`):**
...
agent-observability/
  ...
  collector/            otelcol-contrib config.yaml + launchd plist (local dev only)
  config/               sample .codex/config.toml, .claude/settings.json
  architecture/         LikeC4 source — renders to docs/architecture/*.svg
  docs/                 design brief + rendered architecture diagrams + ADRs
```

**Per-component verification commands** (also encoded in
`§ Evidence` and `.claude/agents/implementer.md`):
...
spec MUST also name a `### Runtime verification` step that exercises the
...
- **Linear** (planning ledger): project `agent_observability` under the
  `VoidAxiom` team. One issue per packet, `VOI-N` identifier, in its
  milestone (M0 Walking skeleton → M5 Polish + companions). `Closes VOI-N`
  in the PR body auto-transitions on squash-merge.
- **GitHub** (delivery ledger): `VoidAxiom/agent-observability` (public).
  Main is `main`; squash-merges only.

**Out of scope (do NOT add):**

- Cost / per-token billing modeling — see spec.
- A Rust daemon in any role — Transport=A means no Rust. If a future
  decision needs Rust we revisit the spec.
- Multi-environment / staging / prod-deploy hygiene — see
  `§ Scope: production-realistic, NOT production-deployed`.
- Langfuse, OpenLLMetry SDKs, or any other observability backend besides
  ClickHouse (+ optional SigNoz read-only companion).

**Secrets.** `.env` (gitignored) carries dev defaults — ClickHouse
credentials, optional OpenAI key if a packet legitimately needs it. Never
commit/log; never spend external API keys without explicit go-ahead.

**Project memory.** `~/.claude/projects/<encoded-key>/memory/` carries
durable notes (user preferences, project context, references to external
resources). Indexed by `MEMORY.md`; one file per memory.
