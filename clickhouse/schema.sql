-- OTel ClickHouse exporter v0.115.0 default schema, pinned for
-- agent-observability VOI-306. Idempotent: every CREATE uses
-- IF NOT EXISTS so re-applying schema.sql is a no-op.
-- Engine: MergeTree() (single-node local).
-- TTL: 7 DAY DELETE per spec (upstream ships 180 DAY).
...
TTL toDateTime(TimeUnix) + INTERVAL 7 DAY DELETE
SETTINGS index_granularity = 8192, ttl_only_drop_parts = 1;
